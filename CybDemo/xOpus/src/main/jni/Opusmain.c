...
Java_com_ione_opustool_OpusJni_Opusencode(JNIEnv *env, jobject thiz, jshortArray lin, jint offset,
    ...
    int i;
    int nbBytes;
    unsigned short cbits[600];
    unsigned short out[200];
    ...
    nbBytes = opus_encode(encoder, out, size, cbits, 600);//
//设定压缩完的数据到数组中
...
jint Java_com_ione_opustool_OpusJni_Opusdecode(JNIEnv *env, jobject thiz,
    ...
    int frame_size = 4800;
    int decode_size = 0;
    unsigned short out[1600];
    unsigned short deout[1600];
    if (decoder == NULL) {
        return 0;
    }

    (*env)->GetByteArrayRegion(env, encoded, 0, size, out);
    decode_size = opus_decode(decoder, out, size, deout, frame_size, 0);
    if (decode_size < 0) {
        return 0;
    }
    int nSize = decode_size * CHANNELS;
