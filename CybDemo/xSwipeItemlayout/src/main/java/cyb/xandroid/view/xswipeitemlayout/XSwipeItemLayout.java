...
public class XSwipeItemLayout extends FrameLayout {
    private static final String TAG = XSwipeItemLayout.class.getSimpleName();
    ...
    // 手动拖动打开和关闭代理
    private XSwipeItemLayoutDelegate mDelegate;
    ...
    public XSwipeItemLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public XSwipeItemLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.XSwipeItemLayout);
    ...
    private void initAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.XSwipeItemLayout_swipeDirection) {
            // 默认向左滑动
        ...
        } else if (attr == R.styleable.XSwipeItemLayout_bottomMode) {
            // 默认是拉出
        ...
        } else if (attr == R.styleable.XSwipeItemLayout_springDistance) {
            // 弹簧距离，不能小于0，默认值为0
        ...
        } else if (attr == R.styleable.XSwipeItemLayout_swipeAble) {
    ...
    public void setSwipeItemLayoutListener(XSwipeItemLayoutDelegate delegate) {
        mDelegate = delegate;
    }
    ...
    protected void onFinishInflate() {
        ...
        if (getChildCount() != 2) {
            throw new IllegalStateException(XSwipeItemLayout.class.getSimpleName() + "必须有且只有两个子控件");
        }
        ...
        mBottomView.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                if (mBottomView.getMeasuredWidth() > 0) {
    ...
    private void performAdapterViewItemClick() {
        ...
        if (t instanceof AdapterView) {
            ...
            int p = view.getPositionForView(XSwipeItemLayout.this);
            if (p != AdapterView.INVALID_POSITION) {
                view.performItemClick(view.getChildAt(p - view.getFirstVisiblePosition()),
                        p, view.getAdapter().getItemId(p));
            }
        }
    }
    ...
    private boolean performAdapterViewItemLongClick() {
        ...
        if (t instanceof AdapterView) {
            ...
            try {
                ...
                handled = (boolean) m.invoke(view, XSwipeItemLayout.this, p, vId);

            } catch (Exception e) {
                e.printStackTrace();

                if (view.getOnItemLongClickListener() != null) {
                    handled = view.getOnItemLongClickListener().onItemLongClick(view, XSwipeItemLayout.this, p, vId);
                }
                if (handled) {
...
//        int childWidth = mBottomView.getMeasuredWidth();
//        int childHeight = mBottomView.getMeasuredHeight();
    }
     ...
     */
    private synchronized void smoothSlideTo(int isOpen) {
    ...
    private void slideTo(int isOpen) {
        if (isOpen == 1) {
            ...
            if (mDelegate != null) {
                mDelegate.onStartOpen(this);
                mDelegate.onOpened(this);
            }
        } else {
            mBottomView.setVisibility(INVISIBLE);
            mCurrentStatus = Status.Closed;
            if (mDelegate != null) {
                mDelegate.onClosed(this);
            }
        }
        mPreStatus = mCurrentStatus;
    ...
    private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback() {
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            ViewCompat.postInvalidateOnAnimation(XSwipeItemLayout.this);
        }
    ...
    private void dispatchSwipeEvent() {
        ...
        //打开和移动是显示mBottomView
        if (mCurrentStatus == Status.Closed && getVisibility() == VISIBLE) {
            mBottomView.setVisibility(INVISIBLE);
        } else if (mBottomView.getVisibility() != VISIBLE) {
            mBottomView.setVisibility(VISIBLE);
        }
    ...
    public interface XSwipeItemLayoutDelegate {
        /**
         * 变为打开状态
         *
         * @param swipeItemLayout
         */
        void onOpened(XSwipeItemLayout swipeItemLayout);
         ...
         * @param swipeItemLayout
         */
        void onClosed(XSwipeItemLayout swipeItemLayout);
         ...
         * @param swipeItemLayout
         */
        void onStartOpen(XSwipeItemLayout swipeItemLayout);
    }
