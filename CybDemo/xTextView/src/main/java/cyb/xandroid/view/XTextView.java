...
import cyb.xandroid.view.decorate.PressDecorator;
...
public class XTextView extends AppCompatTextView {
    ...
    private Decorator pressDecorator;
    ...
    private int pressBgColor = 0XFFD3D3D3;
    private int pressTextColor = 0XFFD3D3D3;

    private boolean drawableAsBackground;
    private BitmapShader drawableBackgroundShader;

    private List<Decorator> decoratorList = new ArrayList<>();
    private List<Decorator> touchDecorators = new ArrayList<>();
    ...
    private void initAttrs(AttributeSet attrs) {
        if (attrs != null) {
            TypedArray typedArray =
                    getContext().obtainStyledAttributes(attrs, R.styleable.XTextView);
            corner = typedArray.getDimension(R.styleable.XTextView_corner, DEFAULT_CORNER);
            leftTopCornerEnable =
                    typedArray.getBoolean(R.styleable.XTextView_left_top_corner, false);
            rightTopCornerEnable =
                    typedArray.getBoolean(R.styleable.XTextView_right_top_corner, false);
            leftBottomCornerEnable =
                    typedArray.getBoolean(R.styleable.XTextView_left_bottom_corner, false);
            rightBottomCornerEnable =
                    typedArray.getBoolean(R.styleable.XTextView_right_bottom_corner, false);
            solid = typedArray.getColor(R.styleable.XTextView_solid, DEFAULT_SOLID);
            strokeWidth = typedArray.getDimension(R.styleable.XTextView_stroke_width,
                    DEFAULT_STROKE_WIDTH);
            strokeColor =
                    typedArray.getColor(R.styleable.XTextView_stroke_color, DEFAULT_STROKE_COLOR);
            drawable = typedArray.getDrawable(R.styleable.XTextView_state_drawable);
            drawableWidth =
                    typedArray.getDimension(R.styleable.XTextView_state_drawable_width, 0);
            drawableHeight =
                    typedArray.getDimension(R.styleable.XTextView_state_drawable_height, 0);
            drawablePaddingLeft =
                    typedArray.getDimension(R.styleable.XTextView_state_drawable_padding_left, 0);
            drawablePaddingTop =
                    typedArray.getDimension(R.styleable.XTextView_state_drawable_padding_top, 0);
            drawable2 = typedArray.getDrawable(R.styleable.XTextView_state_drawable2);
            drawable2Width =
                    typedArray.getDimension(R.styleable.XTextView_state_drawable2_width, 0);
            drawable2Height =
                    typedArray.getDimension(R.styleable.XTextView_state_drawable2_height, 0);
            drawable2PaddingLeft =
                    typedArray.getDimension(R.styleable.XTextView_state_drawable2_padding_left, 0);
            drawable2PaddingTop =
                    typedArray.getDimension(R.styleable.XTextView_state_drawable2_padding_top, 0);
            isShowState = typedArray.getBoolean(R.styleable.XTextView_isShowState, false);
            drawableAsBackground =
                    typedArray.getBoolean(R.styleable.XTextView_drawableAsBackground, false);
            isShowState2 = typedArray.getBoolean(R.styleable.XTextView_isShowState2, false);
            stateDrawableMode = typedArray.getInteger(R.styleable.XTextView_state_drawable_mode,
                    DEFAULT_STATE_DRAWABLE_MODE);
            stateDrawable2Mode = typedArray.getInteger(R.styleable.XTextView_state_drawable2_mode,
                    DEFAULT_STATE_DRAWABLE_MODE);
            textStroke = typedArray.getBoolean(R.styleable.XTextView_text_stroke, false);
            textStrokeColor = typedArray.getColor(R.styleable.XTextView_text_stroke_color,
                    DEFAULT_TEXT_STROKE_COLOR);
            textFillColor = typedArray.getColor(R.styleable.XTextView_text_fill_color,
                    DEFAULT_TEXT_FILL_COLOR);
            textStrokeWidth = typedArray.getDimension(R.styleable.XTextView_text_stroke_width,
                    DEFAULT_TEXT_STROKE_WIDTH);
            autoAdjust = typedArray.getBoolean(R.styleable.XTextView_autoAdjust, false);
            shaderStartColor =
                    typedArray.getColor(R.styleable.XTextView_shaderStartColor, 0);
            shaderEndColor =
                    typedArray.getColor(R.styleable.XTextView_shaderEndColor, 0);
            shaderMode = typedArray.getInteger(R.styleable.XTextView_shaderMode, 0);
            shaderEnable = typedArray.getBoolean(R.styleable.XTextView_shaderEnable, false);
            pressBgColor = typedArray.getColor(R.styleable.XTextView_pressBgColor, Color.TRANSPARENT);
            pressTextColor = typedArray.getColor(R.styleable.XTextView_pressTextColor, pressTextColor);
            typedArray.recycle();
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        isNeedToAdjust(canvas, Decorator.Opportunity.BEFORE_DRAWABLE);
        drawStateDrawable(canvas);
        isNeedToAdjust(canvas, Decorator.Opportunity.BEFORE_TEXT);
        if (textStroke) {
        ...
        super.onDraw(canvas);
        isNeedToAdjust(canvas, Decorator.Opportunity.AT_LAST);
    }
    ...
    /**
     * @return 返回SuperTextView的圆角信息 {@link XTextView#getCorners(float)}.
     */
    ...
    private void checkPressColor(Canvas canvas) {
        if (pressBgColor != Color.TRANSPARENT || pressTextColor != -99) {
            if (pressDecorator == null) {
                pressDecorator = new PressDecorator(pressBgColor)
                        .setPressTextColor(pressTextColor)
                        .setType(Decorator.TYPE_SYSTEM);
                decoratorList.add(0, pressDecorator);
                SYSTEM_ADJUSTER_SIZE++;
            }
            ((PressDecorator) pressDecorator).setPressTextColor(pressTextColor);
            ((PressDecorator) pressDecorator).setPressBgColor(pressBgColor);
        }
    }
    ...
    private void isNeedToAdjust(Canvas canvas, Decorator.Opportunity currentOpportunity) {

        for (int i = 0; i < decoratorList.size(); i++) {
            Decorator decorator = decoratorList.get(i);
            if (currentOpportunity == decorator.getOpportunity()) {
                if (decorator.getType() == Decorator.TYPE_SYSTEM) {
                    decorator.decorate(this, canvas);
                } else if (autoAdjust) {
                    decorator.decorate(this, canvas);
                }
            }
     ...
     */
    public XTextView setCorner(float corner) {
        this.corner = corner;
        postInvalidate();
     ...
     * @return SuperTextView
     */
    public XTextView setSolid(int solid) {
        this.solid = solid;
        postInvalidate();
     ...
     * @return SuperTextView
     */
    public XTextView setStrokeWidth(float strokeWidth) {
     ...
     * @return SuperTextView
     */
    public XTextView setStrokeColor(int strokeColor) {
    ...
    /**
     * 该方法或许在后面版本会移除，请尽快使用{@link XTextView#addAdjuster(Decorator)}来添加一个Adjuster。
     *
     * @param decorator 添加一个Adjuster。{@link XTextView#addAdjuster(Decorator)}
     *                  注意最多支持添加3个Adjuster，否则新的Adjuster总是会覆盖最后一个Adjuster。
     *                  {@link Decorator}。会触发一次重绘。
     * @return SuperTextView
     */
    @Deprecated
    public XTextView setAdjuster(Decorator decorator) {

        return addAdjuster(decorator);
    }

    /**
     * @return 获得最后一个Adjuster，如果存在的话。
     */
    public Decorator getAdjuster() {
        if (decoratorList.size() > SYSTEM_ADJUSTER_SIZE) {
            return decoratorList.get(decoratorList.size() - 1);
        }
        return null;
    }
     ...
     * @return SuperTextView
     */
    public XTextView addAdjuster(Decorator decorator) {
        if (decoratorList.size() < SYSTEM_ADJUSTER_SIZE + ALLOW_CUSTOM_ADJUSTER_SIZE) {
            decoratorList.add(decorator);
        } else {
            decoratorList.remove(decoratorList.size() - 1);
            decoratorList.add(decorator);
        }
        postInvalidate();
        return this;
     ...
     */
    public Decorator removeAdjuster(int index) {
        int realIndex = SYSTEM_ADJUSTER_SIZE + index;
        if (realIndex > SYSTEM_ADJUSTER_SIZE - 1 && realIndex < decoratorList.size()) {
            Decorator remove = decoratorList.remove(realIndex);
            postInvalidate();
            return remove;
        }
        return null;
     ...
     */
    public Decorator getAdjuster(int index) {
        int realIndex = SYSTEM_ADJUSTER_SIZE + index;
        if (realIndex > SYSTEM_ADJUSTER_SIZE - 1 && realIndex < decoratorList.size()) {
            return decoratorList.remove(realIndex);
        }
        return null;
    }
     ...
     * @return SuperTextView
     */
    public XTextView setTextStroke(boolean textStroke) {
     ...
     * @return SuperTextView
     */
    public XTextView setTextStrokeColor(int textStrokeColor) {
     ...
     * @return SuperTextView
     */
    public XTextView setTextFillColor(int textFillColor) {
     ...
     * @return SuperTextView
     */
    public XTextView setTextStrokeWidth(float textStrokeWidth) {
     ...
     * @return SuperTextView
     */
    public XTextView setAutoAdjust(boolean autoAdjust) {
     ...
     * @return SuperTextView
     */
    public XTextView setLeftTopCornerEnable(boolean leftTopCornerEnable) {
     ...
     * @return SuperTextView
     */
    public XTextView setRightTopCornerEnable(boolean rightTopCornerEnable) {
     ...
     * @return SuperTextView
     */
    public XTextView setLeftBottomCornerEnable(boolean leftBottomCornerEnable) {
     ...
     * @return SuperTextView
     */
    public XTextView setRightBottomCornerEnable(boolean rightBottomCornerEnable) {
     ...
     * @return SuperTextView
     */
    public XTextView setDrawable(Drawable drawable) {
     ...
     * @return SuperTextView
     */
    public XTextView setDrawable2(Drawable drawable) {
     ...
     * @return SuperTextView
     */
    public XTextView setDrawable(int drawableRes) {
     ...
     * @return SuperTextView
     */
    public XTextView setDrawable2(int drawableRes) {
     ...
     * @return SuperTextView
     */
    public XTextView setShowState(boolean showState) {
     ...
     * @return SuperTextView
     */
    public XTextView setDrawableAsBackground(boolean drawableAsBackground) {
     ...
     * @return SuperTextView
     */
    public XTextView setShowState2(boolean showState) {
     ...
     * @return SuperTextView
     */
    public XTextView setStateDrawableMode(int stateDrawableMode) {
     ...
     * @return SuperTextView
     */
    public XTextView setStateDrawable2Mode(int stateDrawableMode) {
     ...
     * @return SuperTextView
     */
    public XTextView setDrawableWidth(float drawableWidth) {
     ...
     * @return SuperTextView
     */
    public XTextView setDrawable2Width(float drawableWidth) {
     ...
     * @return SuperTextView
     */
    public XTextView setDrawableHeight(float drawableHeight) {
     ...
     * @return SuperTextView
     */
    public XTextView setDrawable2Height(float drawableHeight) {
     ...
     * @return SuperTextView
     */
    public XTextView setDrawablePaddingLeft(float drawablePaddingLeft) {
     ...
     * @return SuperTextView
     */
    public XTextView setDrawable2PaddingLeft(float drawablePaddingLeft) {
     ...
     * @return SuperTextView
     */
    public XTextView setDrawablePaddingTop(float drawablePaddingTop) {
     ...
     * @return SuperTextView
     */
    public XTextView setDrawable2PaddingTop(float drawablePaddingTop) {
    ...
    /**
     * @param shaderStartColor 设置渐变起始色。需要调用{@link XTextView#setShaderEnable(boolean)}后才能生效。会触发一次重绘。
     * @return SuperTextView
     */
    public XTextView setShaderStartColor(int shaderStartColor) {
    ...
    /**
     * @param shaderEndColor 设置渐变结束色。需要调用{@link XTextView#setShaderEnable(boolean)}后才能生效。会触发一次重绘。
     * @return SuperTextView
     */
    public XTextView setShaderEndColor(int shaderEndColor) {
     ...
     * @return SuperTextView
     */
    public XTextView setShaderMode(int shaderMode) {
     ...
     * @return SuperTextView
     */
    public XTextView setShaderEnable(boolean shaderEnable) {
     ...
     * @param pressBgColor
     */
    public XTextView setPressBgColor(int pressBgColor) {
     ...
     */
    public XTextView setPressTextColor(int pressTextColor) {
     ...
     * @return SuperTextView
     */
    public XTextView setFrameRate(int frameRate) {
        if (frameRate > 0) {
    ...
    /**
     * 启动动画。需要设置{@link XTextView#setAutoAdjust(boolean)}为true才能看到。
     */
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (actionMasked == MotionEvent.ACTION_DOWN) {
            for (int i = 0; i < decoratorList.size(); i++) {
                Decorator decorator = decoratorList.get(i);
                if (decorator.onTouch(this, event)) {
                    if (decorator.type == Decorator.TYPE_SYSTEM || isAutoAdjust()) {
                        hasConsume = true;
                        touchDecorators.add(decorator);
                    }
                }
            }
            superTouchEvent = super.onTouchEvent(event);
        } else {
            for (int i = 0; i < touchDecorators.size(); i++) {
                Decorator decorator = touchDecorators.get(i);
                decorator.onTouch(this, event);
                hasConsume = true;
            }
            if (superTouchEvent) {
     ...
     * 这具有非常重要的意义。比如，默认实现的{@link DefaultDecorator}能够动态的调整文字的大小。
     * 当然，你可以用它来实现各种各样的效果。
     * 你可以指定Adjuster的作用层级，通过调用{@link Decorator#setOpportunity(Opportunity)}，
    ...
    public static abstract class Decorator {
         ...
         */
        protected abstract void decorate(XTextView v, Canvas canvas);

        /**
         * 在这个方法中，你能够捕获到SuperTextView中发生的触摸事件。
         * 需要注意，如果你在该方法中返回了true来处理SuperTextView的触摸事件的话，你将在
         * SuperTextView的setOnTouchListener中设置的OnTouchListener中同样能够捕获到这些触摸事件，
         * 即使你在OnTouchListener中返回了false。
         * 但是，如果你在OnTouchListener中返回了true，这个方法将会失效，因为事件在OnTouchListener中被拦截了。
         *
         * @param v     SuperTextView
         * @param event 控件件接收到的触摸事件。
         * @return 默认返回false。如果想持续的处理控件的触摸事件就返回true。
         * 否则，只能接收到{@link MotionEvent#ACTION_DOWN}事件。
         */
        public boolean onTouch(XTextView v, MotionEvent event) {
            return false;
        }
         ...
         */
        public Decorator setOpportunity(Opportunity opportunity) {
         ...
         * @hide
         */
        private Decorator setType(int type) {
            this.type = type;
            return this;
        }
         ...
         * 你可以通过{@link Decorator#setOpportunity(Opportunity)}来指定Adjuster的绘制层级。
    ...
    public static class DefaultDecorator extends Decorator {

        @Override
        public void decorate(XTextView v, Canvas canvas) {
            int length = v.length();
